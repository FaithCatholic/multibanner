...
/**
 * Provides a multibanner deletion confirmation form.
 */
class DeleteMultiple extends ConfirmFormBase {

  /**
   * The array of multibanner entities to delete.
   *
   * @var string[][]
   */
  ...
  public function __construct(PrivateTempStoreFactory $temp_store_factory, EntityTypeManagerInterface $manager) {
    ...
    $this->storage = $manager->getStorage('multibanner');
  }
  ...
  public function getFormId() {
    return 'multibanner_multiple_delete_confirm';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $this->entityInfo = $this->tempStoreFactory->get('multibanner_multiple_delete_confirm')->get(\Drupal::currentUser()->id());
    if (empty($this->entityInfo)) {
      return new RedirectResponse($this->getCancelUrl()->setAbsolute()->toString());
    }
    /** @var \Drupal\multibanner\MultibannerInterface[] $entities */
    ...
    foreach ($this->entityInfo as $id => $langcodes) {
      foreach ($langcodes as $langcode) {
        ...
        if (count($languages) > 1 && $entity->isDefaultTranslation()) {
          ...
          $items[$default_key] = [
            'label' => [
              '#markup' => $this->t('@label (Original translation) - <em>The following multibanner translations will be deleted:</em>', ['@label' => $entity->label()]),
            ],
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    if ($form_state->getValue('confirm') && !empty($this->entityInfo)) {
      ...
      if ($delete_entities) {
        ...
        $this->logger('multibanner')->notice('Deleted @count multibanner entities.', ['@count' => count($delete_entities)]);
      }
      ...
      if ($delete_translations) {
        ...
        if ($count) {
          $total_count += $count;
          $this->logger('multibanner')->notice('Deleted @count multibanner translations.', ['@count' => $count]);
        }
      }

      if ($total_count) {
        \Drupal\Core\Messenger\MessengerInterface::addMessage($this->formatPlural($total_count, 'Deleted 1 multibanner.', 'Deleted @count multibanner entities.'));
      }

      $this->tempStoreFactory->get('multibanner_multiple_delete_confirm')->delete(\Drupal::currentUser()->id());
    }
