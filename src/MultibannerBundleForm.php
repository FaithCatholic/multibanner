 ...
 */
class MultibannerBundleForm extends EntityForm {
  ...
  /**
   * Manager for multibanner type plugins.
   *
   * @var \Drupal\multibanner\MultibannerTypeManager
   */
  protected $multibannerTypeManager;
   ...
   *
   * @param \Drupal\multibanner\MultibannerTypeManager $multibanner_type_manager
   *   Multibanner type manager.
   */
  public function __construct(MultibannerTypeManager $multibanner_type_manager, EntityFieldManagerInterface $entity_field_manager) {
    $this->multibannerTypeManager = $multibanner_type_manager;
  ...
  public static function create(ContainerInterface $container) {
    return new static(
      $container->get('plugin.manager.multibanner.type'),
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    if ($this->operation == 'add') {
      $form['#title'] = $this->t('Add multibanner bundle');
    }
    elseif ($this->operation == 'edit') {
      $form['#title'] = $this->t('Edit %label multibanner bundle', ['%label' => $bundle->label()]);
    }
    ...
    $form['label'] = [
      ...
      '#description' => $this->t('The human-readable name of this multibanner bundle.'),
      '#required' => TRUE,
      '#size' => 30,
      '#weight' => -100,
    ];
    ...
    $form['id'] = [
      ...
      '#machine_name' => [
        'exists' => ['\Drupal\multibanner\Entity\MultibannerBundle', 'exists'],
        'source' => ['label'],
      ],
      '#description' => $this->t('A unique machine-readable name for this multibanner bundle.'),
      '#weight' => -90,
    ];
    ...
    $form['description'] = [
      ...
      '#description' => $this->t('Describe this multibanner bundle. The text will be displayed on the <em>Add new multibanner</em> page.'),
      '#weight' => -80,
    ];

    $plugins = $this->multibannerTypeManager->getDefinitions();
    $options = [];
    ...
    $form['type'] = [
      ...
      '#options' => $options,
      '#description' => $this->t('Multibanner type provider plugin that is responsible for additional logic related to this multibanner.'),
      '#weight' => -70,
      '#ajax' => [
    ...
    // Multibanner type plugin configuration.
    ...
    /** @var \Drupal\multibanner\MultibannerTypeInterface $plugin */
    if ($plugin = $bundle->getType()) {
      ...
      /** @var \Drupal\multibanner\MultibannerTypeBase $instance */
      $instance = $this->multibannerTypeManager->createInstance($plugin->getPluginId(), $plugin_configuration);
    ...
    $form['field_mapping'] = [
      ...
      'description' => [
        '#type' => 'html_tag',
        '#tag' => 'p',
        '#value' => $this->t('Multibanner type plugins can provide metadata fields such as title, caption, size information, credits, ... multibanner can automatically save this metadata information to entity fields, which can be configured below. Information will only be mapped if the entity field is empty.'),
      ],
      '#weight' => -50,
    ];
    ...
    if (empty($plugin) || empty($plugin->providedFields())) {
      ...
      foreach ($this->entityFieldManager->getFieldDefinitions('multibanner', $bundle->id()) as $field_name => $field) {
    ...
    $form['additional_settings'] = [
      '#type' => 'vertical_tabs',
      '#attached' => [
        'library' => ['multibanner/multibanner_bundle_form'],
      ],
      '#weight' => 100,
    ];
    ...
    $form['workflow']['options']['new_revision']['#description'] = $this->t('Automatically create a new revision of multibanner entities. Users with the Administer multibanner permission will be able to override this option.');
    ...
    if ($this->moduleHandler->moduleExists('language')) {
      ...
      $language_configuration = ContentLanguageSettings::loadByEntityTypeBundle('multibanner', $bundle->id());
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    $instance = $this->multibannerTypeManager->createInstance($plugin, $plugin_configuration);
  ...
  protected function actions(array $form, FormStateInterface $form_state) {
    ...
    $actions['submit']['#value'] = $this->t('Save multibanner bundle');
    $actions['delete']['#value'] = $this->t('Delete multibanner bundle');
  ...
  protected function copyFormValuesToEntity(EntityInterface $entity, array $form, FormStateInterface $form_state) {
    ...
    /** @var \Drupal\multibanner\MultibannerBundleInterface $entity */
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    $fields = $this->entityFieldManager->getFieldDefinitions('multibanner', $bundle->id());
    $multibanner = $this->entityTypeManager->getStorage('multibanner')->create(array('bundle' => $bundle->id()));
    $value = (bool) $form_state->getValue(['options', 'status']);
    if ($multibanner->status->value != $value) {
